...
export const useMenuStore = defineStore('menu', () => {
    ...
    function reset(){
      graphStore.renderer.reset();
    };
    function menu_inserirDesfazer(){
      graphStore.menu_inserirDesfazer();
    };
    function menu_inserir(event){
      const { shiftKey, ctrlKey } = event;
      graphStore.menu_inserir(null, shiftKey, ctrlKey);
    };
    function menu_dados(event){
      const { shiftKey } = event;
      graphStore.menu_dados(shiftKey, null);
    };
    ...
    function menu_botaoAbre(event) {
      const { shiftKey } = event;
      menu_carregaJSONNavegador(shiftKey ? '': 'salvo_pelo_botao');
    };
    ...
    function menu_botaoSalva(event) { //xxx
      const { shiftKey } = event;
      var r = menu_salvaJSONNavegador(shiftKey? '': 'salvo_pelo_botao');
      if (r) {
    ...
    function menu_copiaItensParaOutraAba(bNovaJanela, bFocusNoDestino) { 
      ...
      if (bNovaJanela) {
        graphStore.abaFilha = window.open(graphStore.base); 
      }
